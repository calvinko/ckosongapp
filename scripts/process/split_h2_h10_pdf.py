"""
TAKEN FROM split_pdf.py - updated to fit new h2-10 pdfs
...
def _get_splits_from_song_list(book_name, pages: list[PageObject]) -> Dict:
    parent_dir = Path(__file__).resolve().parent.parent.parent
    with open(Path.joinpath(parent_dir, "lib/chineseSongList.json"), "r") as f:
        song_list = json.loads(f.read())["songs"]
    
    book_num_set = set()
    book_num_list = []
    song_slug = []
    for song in song_list:
        if song["hymn"] == book_name:
            book_num_list.append(int(song["pagenum"]))
            song_slug.append(song["slug"])
            book_num_set.add(int(song["pagenum"]))

    # add last page
    # import ipdb; ipdb.set_trace()
    book_num_set.add(len(pages) + 1)
    book_num_list.append(len(pages) + 1)
    
    res = []
    sorted_set = list(book_num_set)
    sorted_set.sort()
    paired_pages = list(pairwise(sorted_set))
    for i, start_page in enumerate(book_num_list):
        for (sp, ep) in paired_pages:
            if sp == start_page:
                res.append({
                    "pair": (sp, ep),
                    "slug": song_slug[i]
